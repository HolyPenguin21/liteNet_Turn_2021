...
public class AiSolo : AiBehaviour
{
    ...
    private IEnumerator Attack_NearbyEnemy(Character character)
    {
        ...
        CharAttack a_Attack = attackCalculation.Get_MaxDmgAttack(character, t_Hex.character);
        int a_AttackId = attackCalculation.Get_AttackId(character, a_Attack);

        CharAttack t_Attack = attackCalculation.Get_ReturnAttack(character, a_AttackId, t_Hex.character);
    ...
    private Hex Get_FreeHirePoint(Character aiHero)
    {
        ...
        Hex closestHireHex = Get_ClosestHirePoint(hireHexes, aiHero);
        if (closestHireHex == null) return null;
        if (aiHero.hex == closestHireHex) return closestHireHex;

        List<Hex> generalPath = sceneMain_ui.pathfinding.Get_Path(aiHero.hex, closestHireHex);
        if (generalPath == null || generalPath.Count == 0) return null;

        List<Hex> realPath = sceneMain_ui.pathfinding.Get_RealPath(aiHero, generalPath);
        if (realPath == null || realPath.Count == 0) return null;

        int pathCost = sceneMain_ui.pathfinding.Get_PathCost(aiHero, realPath);
        if (aiHero.movement.movePoints_cur >= pathCost)
        {
    ...
    private Hex Get_ClosestHirePoint(List<Hex> hireHexes, Character aiHero)
    {
        int curPathCost = 99;
        ...
        for(int x = 0; x < hireHexes.Count; x++)
        {
            ...
            int pathCost = sceneMain_ui.pathfinding.Get_PathCost(aiHero, realPath);
            Debug.Log("curPathCost : " + curPathCost + " pathCost : " + pathCost);
            if(pathCost < curPathCost)
            {
